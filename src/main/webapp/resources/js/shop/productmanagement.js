$(function () {
    ...
    function getList() {
        ...
        $.getJSON(listUrl, function (data) {
            if (data.success) {
                ...
                // 商品名称、优先级、上架/下架（含productId）、编辑按钮（含 productId）、预览（含productId）
                productList.map(function (item, index) {
                    var textOp = '下架';
                    var contraryStatus = 0;
                    if (item.enableStatus == 0) {
                        // 若状态值为 0，表明是已下架的商品，操作变为上架（即点击上架按钮上架相关商品）
                        textOp = "上架";
                        contraryStatus = 1;
                    } else {
                        contraryStatus = 0;
    ...
    // 将 class 为 product-wrap 里面的 a 标签绑定上点击的事件
    $('.product-wrap')
        .on(
            'click',
            'a',
    ...
    function changeItemStatus(id, enableStatus) {
        // 定义product json 对象并添加 productId 以及状态（上架/下架）
        var product = {};
